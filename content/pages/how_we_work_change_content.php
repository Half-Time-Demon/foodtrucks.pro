...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_GET['id'])) {
        ...
        $query = "UPDATE how_we_work SET title='$title', text='$text', image_name='$image_name' WHERE id=$id";
        ...
        $query = "INSERT INTO how_we_work (title, text, image_name) VALUES ('$title', '$text', '$image_name')";
...
<h2 class="how_we_work_change_content">how we work change content</h2>
    <div class="container">
    	...
    	<div class="table">
    		...
    		<div class="table-content">	
                <?php
                    $result= mysqli_query($connection,"SELECT * FROM `how_we_work`");
                        ...
                        echo '
                            <div class="table-row">		
                                ...
                                <div class="table-data">
                                    <a href="'.$config['site_url'].'includes/how_we_work_change_content_process.php?id='.$line['id'].'">
                                        <img src="'.$config['site_url'].'content/pictures/icons/redagate.png" alt="">
                                    </a>
                                    <a href="'.$config['site_url'].'includes/delete_how_we_work.php?id='.$line['id'].'">
    ...
    <form action="<?php echo $config['site_url'] ?>content/pages/admin_panel.php<?php if (isset($_GET['id'])) { echo '?id=' . $_GET['id']; } ?>" method="post">
        ...
        if (isset($_GET['id'])) {
            $id = $_GET['id'];
            $result = mysqli_query($connection, "SELECT * FROM how_we_work WHERE id = $id");
        ...
        <input type="text" name="image_name" class="rounded-input border-bottom-input border-removed-input border-customized-input" value="<?php if (isset($_GET['id'])) { echo $value['title']; } ?>" required>
        </br></br>
